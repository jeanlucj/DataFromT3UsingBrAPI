---
title: "Cross validation for the trials- for looping for factor with spatial analysis only
  analysis"
...
if(length(out_ind) == 0){
 ...
 SL = droplevels(SL)
  ...
  H2_Trait = rbind(H2_Trait,h2) # store the heritability in the tibble 
    ...
    RM = cbind(location = paste(env),Trait = paste(Trait),RMSE = round(RMSE(obs[,paste(Trait)], prd[,paste(Trait)], na.rm = T),3),
               MAE = round(MAE(obs[,paste(Trait)], prd[,paste(Trait)], na.rm = T),3))
